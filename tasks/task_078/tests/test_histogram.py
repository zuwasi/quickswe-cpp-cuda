...
class TestHistogram:
    ...
    def test_medium_array(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '10000', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_total_count(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '50000', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('CPU_TOTAL') == info.get('GPU_TOTAL'), \
            f"CPU_TOTAL={info.get('CPU_TOTAL')} GPU_TOTAL={info.get('GPU_TOTAL')}"

    @pytest.mark.fail_to_pass
    def test_different_seed(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '20000', '--seed', '777'])
        info = parse_output(stdout)
