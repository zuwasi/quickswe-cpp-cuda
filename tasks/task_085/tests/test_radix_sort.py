...
class TestRadixSort:
    ...
    def test_mixed_sorted(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('SORTED') == 1, "Output not sorted"

    @pytest.mark.fail_to_pass
    def test_no_neg_after_pos(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '150', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('NEG_AFTER_POS') == 0, "Negative values after positive"

    @pytest.mark.fail_to_pass
    def test_mixed_seed777(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '200', '--seed', '777'])
        info = parse_output(stdout)
    ...
    def test_positive_only(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100', '--mode', '1', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
