...
class TestJacobiUM:
    ...
    def _compile(self):
        self.binary = compile_cuda(['jacobi_um.cu'], 'jacobi_um')

    @pytest.mark.fail_to_pass
    def test_default(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '64', '--iters', '20', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_more_iters(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '32', '--iters', '50', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_larger(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100', '--iters', '10', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_seed777(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '48', '--iters', '30', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '16', '--iters', '5', '--seed', '1'])
        info = parse_output(stdout)
