...
class TestStreamPipeline:
    ...
    def _compile(self):
        self.binary = compile_cuda(['stream_pipeline.cu'], 'stream_pipeline')
    ...
    @pytest.mark.fail_to_pass
    def test_large(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '50000', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_sum_matches(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '20000', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('SUM_REL_ERROR', 1.0) < 1e-3

    @pytest.mark.fail_to_pass
    def test_seed777(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '15000', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
