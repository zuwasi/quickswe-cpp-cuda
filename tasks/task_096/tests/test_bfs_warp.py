...
class TestBFSWarp:
    ...
    def _compile(self):
        self.binary = compile_cuda(['bfs_warp.cu'], 'bfs_warp')

    @pytest.mark.fail_to_pass
    def test_500_vertices(self):
    ...
    @pytest.mark.fail_to_pass
    def test_1000_vertices(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '1000', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_unvisited_match(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '300', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('CPU_UNVISITED') == info.get('GPU_UNVISITED')

    @pytest.mark.fail_to_pass
    def test_dense_graph(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '200', '--degree', '8', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_sparse_graph(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '500', '--degree', '2', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
