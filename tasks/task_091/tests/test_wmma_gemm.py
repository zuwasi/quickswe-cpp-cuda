...
class TestWMMAGemm:
    ...
    def _compile(self):
        self.binary = compile_cuda(['wmma_gemm.cu'], 'wmma_gemm')

    @pytest.mark.fail_to_pass
    def test_32x32x32(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '32', '--N', '32', '--K', '32', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_64x64x32(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '64', '--N', '64', '--K', '32', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_16x16x16(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '16', '--N', '16', '--K', '16', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
