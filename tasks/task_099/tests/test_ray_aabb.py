...
class TestRayAABB:
    ...
    def _compile(self):
        self.binary = compile_cuda(['ray_aabb.cu'], 'ray_aabb')

    @pytest.mark.fail_to_pass
    def test_default(self):
        stdout, _, rc = run_binary(self.binary, ['--prims', '16', '--rays', '256', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_many_rays(self):
        stdout, _, rc = run_binary(self.binary, ['--prims', '32', '--rays', '512', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_no_missed_hits(self):
        stdout, _, rc = run_binary(self.binary, ['--prims', '16', '--rays', '200', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MISSED_HITS', 999) == 0

    @pytest.mark.fail_to_pass
    def test_hit_count_match(self):
        stdout, _, rc = run_binary(self.binary, ['--prims', '20', '--rays', '300', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('CPU_HITS') == info.get('GPU_HITS')

    @pytest.mark.fail_to_pass
    def test_few_prims(self):
        stdout, _, rc = run_binary(self.binary, ['--prims', '4', '--rays', '128', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--prims', '4', '--rays', '32', '--seed', '1'])
        info = parse_output(stdout)
        assert 'PRIMS' in info and 'MATCH' in info
