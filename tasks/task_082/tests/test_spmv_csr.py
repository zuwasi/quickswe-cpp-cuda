...
class TestSpMVCSR:
    ...
    def _compile(self):
        self.binary = compile_cuda(['spmv_csr.cu'], 'spmv_csr')
    ...
    @pytest.mark.fail_to_pass
    def test_large(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '500', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"
    ...
    def test_empty_rows(self):
        ...
        info = parse_output(stdout)
        assert info.get('EMPTY_ROW_ERRORS', 999) == 0, \
            f"EMPTY_ROW_ERRORS={info.get('EMPTY_ROW_ERRORS')}"
    ...
    def test_dense_rows(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '150', '--nnz', '10', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
