...
class TestMemPool:
    ...
    def _compile(self):
        self.binary = compile_cuda(['mem_pool.cu'], 'mem_pool')

    @pytest.mark.fail_to_pass
    def test_default(self):
        stdout, _, rc = run_binary(self.binary, ['--ops', '20', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_many_ops(self):
        stdout, _, rc = run_binary(self.binary, ['--ops', '40', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_small_alloc(self):
        stdout, _, rc = run_binary(self.binary, ['--ops', '30', '--alloc', '256', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_no_alloc_failures(self):
        stdout, _, rc = run_binary(self.binary, ['--ops', '15', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('ALLOC_FAILURES', 999) == 0

    def test_compilation_succeeds(self):
        assert os.path.isfile(self.binary)

    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--ops', '5', '--seed', '1'])
        info = parse_output(stdout)
        assert 'OPS' in info and 'MATCH' in info
