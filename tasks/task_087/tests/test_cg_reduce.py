...
class TestCGReduce:
    ...
    def _compile(self):
        self.binary = compile_cuda(['cg_reduce.cu'], 'cg_reduce')

    @pytest.mark.fail_to_pass
    def test_tile16(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '5000', '--tile', '16', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"REL_ERROR={info.get('REL_ERROR')}"

    @pytest.mark.fail_to_pass
    def test_tile8(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '5000', '--tile', '8', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"REL_ERROR={info.get('REL_ERROR')}"

    @pytest.mark.fail_to_pass
    def test_tile4(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '3000', '--tile', '4', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_tile16_large(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '50000', '--tile', '16', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100', '--tile', '32', '--seed', '1'])
        info = parse_output(stdout)
        assert 'N' in info
        assert 'MATCH' in info

    def test_tile32_works(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '5000', '--tile', '32', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
