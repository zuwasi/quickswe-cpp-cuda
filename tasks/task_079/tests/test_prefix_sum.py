...
class TestPrefixSum:
    ...
    def test_first_element_zero(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '64', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('FIRST_OK') == 1, f"FIRST_ELEMENT={info.get('FIRST_ELEMENT')}"

    @pytest.mark.fail_to_pass
    def test_size_256(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '256', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_small_size(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '16', '--seed', '7'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
