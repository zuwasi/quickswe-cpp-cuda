...
class TestPersistentKernel:
    ...
    def _compile(self):
        self.binary = compile_cuda(['persistent_kernel.cu'], 'persistent_kernel')

    @pytest.mark.fail_to_pass
    def test_default(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '5000', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}, ZEROS={info.get('ZEROS')}"

    @pytest.mark.fail_to_pass
    def test_large(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '20000', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_no_zeros(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '10000', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('ZEROS', 999) == 0

    @pytest.mark.fail_to_pass
    def test_odd_size(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '7777', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100', '--seed', '1'])
        info = parse_output(stdout)
        assert 'N' in info and 'MATCH' in info

    def test_small_within_one_launch(self):
