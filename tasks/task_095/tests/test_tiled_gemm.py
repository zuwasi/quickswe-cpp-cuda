...
class TestTiledGEMM:
    ...
    def test_64x64x64(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '64', '--N', '64', '--K', '64', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_128x128x64(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '128', '--N', '128', '--K', '64', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_32x64x128(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '32', '--N', '64', '--K', '128', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_64x32x32(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '64', '--N', '32', '--K', '32', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_256x256x256(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '256', '--N', '256', '--K', '256', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '16', '--N', '16', '--K', '16', '--seed', '1'])
        info = parse_output(stdout)
        assert 'M' in info and 'MATCH' in info

    def test_single_tile(self):
        stdout, _, rc = run_binary(self.binary, ['--M', '16', '--N', '16', '--K', '16', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
