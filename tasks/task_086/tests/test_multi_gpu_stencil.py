...
@skip_no_nvcc
class TestMultiGPUStencil:

    @pytest.fixture(autouse=True)
    def _compile(self):
        self.binary = compile_cuda(['multi_gpu_stencil.cu'], 'multi_gpu_stencil')

    @pytest.mark.fail_to_pass
    def test_4_partitions(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '2000', '--parts', '4', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_8_partitions(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '2000', '--parts', '8', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_2_partitions(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '1000', '--parts', '2', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_boundary_errors(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '1500', '--parts', '4', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('BOUNDARY_MISMATCHES', 999) == 0

    @pytest.mark.fail_to_pass
    def test_odd_size(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '1777', '--parts', '3', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '500', '--parts', '1', '--seed', '1'])
        info = parse_output(stdout)
        assert 'N' in info
        assert 'MATCH' in info

    def test_single_partition(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '500', '--parts', '1', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
