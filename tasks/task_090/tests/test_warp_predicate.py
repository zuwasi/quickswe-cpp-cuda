...
@skip_no_nvcc
class TestWarpPredicate:
    @pytest.fixture(autouse=True)
    def _compile(self):
        self.binary = compile_cuda(['warp_predicate.cu'], 'warp_predicate')
    ...
    def test_low_threshold(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '512', '--threshold', '25', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_high_threshold(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '2048', '--threshold', '75', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
