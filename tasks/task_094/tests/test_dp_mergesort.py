...
@skip_no_nvcc
class TestDPMergeSort:
    @pytest.fixture(autouse=True)
    def _compile(self):
        self.binary = compile_cuda(['dp_mergesort.cu'], 'dp_mergesort')

    @pytest.mark.fail_to_pass
    def test_512(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '512', '--seed', '42'])
    ...
    @pytest.mark.fail_to_pass
    def test_2048(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '2048', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
