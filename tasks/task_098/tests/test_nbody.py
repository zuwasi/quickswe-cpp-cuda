...
class TestNBody:
    ...
    def _compile(self):
        self.binary = compile_cuda(['nbody.cu'], 'nbody')

    @pytest.mark.fail_to_pass
    def test_128_bodies(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '128', '--steps', '5', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"MISMATCHES={info.get('MISMATCHES')}"

    @pytest.mark.fail_to_pass
    def test_256_bodies(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '256', '--steps', '3', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_single_step(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '64', '--steps', '1', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_10_steps(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100', '--steps', '10', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_512_bodies(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '512', '--steps', '2', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
    ...
    def test_output_format(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '16', '--steps', '1', '--seed', '1'])
        info = parse_output(stdout)
