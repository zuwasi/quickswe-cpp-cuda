...
class TestReduction:
    ...
    def _compile(self):
        self.binary = compile_cuda(['reduction.cu'], 'reduction')
    ...
    @pytest.mark.fail_to_pass
    def test_100k(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '100000', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1, f"REL_ERROR={info.get('REL_ERROR')}"

    @pytest.mark.fail_to_pass
    def test_512(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '512', '--seed', '13'])
        info = parse_output(stdout)
    ...
    def test_cpu_sum_positive(self):
