...
class TestFFT:
    ...
    def _compile(self):
        self.binary = compile_cuda(['fft_ct.cu'], 'fft_ct')

    @pytest.mark.fail_to_pass
    def test_forward_256(self):
    ...
    @pytest.mark.fail_to_pass
    def test_forward_512(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '512', '--seed', '99'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_forward_1024(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '1024', '--seed', '13'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_inverse_256(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '256', '--inverse', '--seed', '42'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1

    @pytest.mark.fail_to_pass
    def test_forward_64(self):
        stdout, _, rc = run_binary(self.binary, ['--size', '64', '--seed', '777'])
        info = parse_output(stdout)
        assert info.get('MATCH') == 1
